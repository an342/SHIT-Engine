#include "gf3dN_list.h"
#include "simple_logger.h"

void gf3dN_list_delete(List *list)
{
	if (!list)return;
	if (list->elements)
	{
...
List *gf3dN_list_new()
{
	return gf3dN_list_new_size(16);
}

List *gf3dN_list_new_size(Uint32 count)
{
	List *l;
	if (!count)
	{
...
void *gf3dN_list_get_nth(List *list, Uint32 n)
{
	if (!list)
	{
...
List *gf3dN_list_expand(List *list)
{
	List *l;
	if (!list)
	{
	...
	l = gf3dN_list_new_size(list->size * 2);
	if (!l)
	{
		return list;
	}
	...
	l->count = list->count;
	gf3dN_list_delete(list);
	return l;
}

List *gf3dN_list_append(List *list, void *data)
{
	if (!list)
	{
		slog("no list provided");
...
List *gf3dN_list_prepend(List *list, void *data)
{
	return gf3dN_list_insert(list, data, 0);
}

List *gf3dN_list_insert(List *list, void *data, Uint32 n)
{
	if (!list)
	{
		slog("no list provided");
...
List *gf3dN_list_delete_first(List *list)
{
	return gf3dN_list_delete_nth(list, 0);
}

List *gf3dN_list_delete_last(List *list)
{
	if (!list)
	{
		slog("no list provided");
	...
	return gf3dN_list_delete_nth(list, list->count - 1);
}

int gf3dN_list_delete_data(List *list, void *data)
{
	int i;
	if (!list)
	{
...
List *gf3dN_list_delete_nth(List *list, Uint32 n)
{
	if (!list)
	{
		slog("no list provided");
...
Uint32 gf3dN_list_get_count(List *list)
{
	if (!list)return 0;
	return list->count;
}

void gf3dN_list_foreach(List *list, void(*function)(void *data, void *context), void *contextData)
{
	int i;
	if (!list)
	{
